...
import { ArrowLeft, Phone, Mail, Globe, Instagram, Camera, Loader2, RefreshCw, Plus, MessageSquare, Phone as PhoneIcon, Mail as MailIcon, Calendar, FileText, AlertCircle, Activity, Pencil } from "lucide-react";
import { format, formatDistanceToNow } from "date-fns";
import { toast } from "sonner";
import { cn, formatDateOnly, extractClientFields } from "@/lib/utils";
...
const ACTIVITY_TYPES = [
  ...
  ...(ENABLE_PROPOSALS ? [{ value: "PROPOSAL", label: "Proposal", icon: FileText, color: "text-indigo-500" }] : []),
...
export default function ClientDetailPage({ id }: { id: string }) {
  ...
  const [editCategory, setEditCategory] = useState("RETAINER");
  const [editHealth, setEditHealth] = useState("GREEN");
  const [editServiceType, setEditServiceType] = useState("");
  const [editPlatforms, setEditPlatforms] = useState("");
  const [editTargetAudience, setEditTargetAudience] = useState("");
  const [editNotes, setEditNotes] = useState("");
  const [editWebsiteUrl, setEditWebsiteUrl] = useState("");
  const [editSocialHandles, setEditSocialHandles] = useState("");
  ...
  const updateMutation = useUpdateClient({
    mutation: {
      ...
      onError: () => toast.error("Failed to update client details"),
    },
  });
  ...
  const openEditClientModal = () => {
    if (!client) return;
    const parsed = extractClientFields(client);
    setEditCompanyName(client.companyName || "");
    setEditContactPerson(client.contactPerson || "");
    setEditPhone(client.phone || "");
    setEditEmail(client.email || "");
    setEditCategory(client.category || "RETAINER");
    setEditHealth(client.health || "GREEN");
    setEditServiceType(parsed.rawServiceType || (client as any).serviceType || "");
    setEditPlatforms(parsed.rawPlatforms || (client as any).platforms || "");
    setEditTargetAudience(parsed.rawTargetAudience || (client as any).targetAudience || "");
    setEditNotes(parsed.notes || "");
    setEditWebsiteUrl(client.websiteUrl || "");
    setEditSocialHandles(client.socialHandles || "");
  ...
  if (!isAdmin && client.assignedTo !== user?.id) {
    const isAssignedToProject = (allProjects ?? []).some(p => p.clientId === id && p.assignedTo === user?.id);
    if (!isAssignedToProject) {
      return (
        <div className="p-12 text-center">
          <AlertCircle className="h-10 w-10 text-destructive mx-auto mb-3" />
          <h2 className="text-xl font-bold">Access Denied</h2>
          <p className="text-muted-foreground mt-1">You are not assigned to this client or any of its projects.</p>
  ...
  return (
    <div className="p-6 space-y-6 animated-fade-in">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {isAdmin && (
                <Button
                  ...
                  data-testid="edit-client-detail-btn"
                >
          ...
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
              {(() => {
                ...
                return (
                  <>
                    <div className="grid grid-cols-2 gap-3 bg-muted/30 p-3.5 rounded-xl border border-border/50 text-xs">
                      <div>
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-foreground">{client.category || "RETAINER"}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground block text-[10px] uppercase font-bold">Email</span>
                        <span className="font-semibold text-foreground truncate block">{client.email || "—"}</span>
                      </div>
                      <div>
                    ...
                    <div className="pt-2 border-t border-border space-y-1">
                      ...
                      <p className="text-muted-foreground text-xs whitespace-pre-line p-2.5 bg-card border border-border rounded-lg min-h-[60px]">
      ...
      {/* Edit Client Dialog (Admin & Special Privileges Only) */}
      {isAdmin && (
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-lg">
                <Pencil className="h-4 w-4 text-primary" /> Edit Client Details
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="edit-category">Billing Category</Label>
                  <Select value={editCategory} onValueChange={(v) => setEditCategory(v ?? "RETAINER")}>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. Gen Z, B2B Professionals"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label htmlFor="edit-health">Health Status</Label>
                  <Select value={editHealth} onValueChange={(v) => setEditHealth(v ?? "GREEN")}>
                    <SelectTrigger id="edit-health"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="edit-social">Social Handles</Label>
                <Input
                  id="edit-social"
                  value={editSocialHandles}
                  onChange={(e) => setEditSocialHandles(e.target.value)}
                  placeholder="@handle"
                />
              </div>
              <div className="space-y-1.5 pt-2 border-t border-border">
                <Label htmlFor="edit-notes" className="font-semibold text-foreground">Notes & Directives</Label>
                <Textarea
                  id="edit-notes"
                  value={editNotes}
                  onChange={(e) => setEditNotes(e.target.value)}
                  placeholder="Client instructions, brand guidelines, important reminders, internal comments..."
                  rows={3}
                />
              </div>
            </div>
            <DialogFooter>
              ...
              <Button
                ...
                disabled={!editCompanyName.trim() || updateMutation.isPending}
              >

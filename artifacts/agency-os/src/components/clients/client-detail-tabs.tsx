    ...
    <Tabs defaultValue="overview" className="space-y-6">
          ...
          <Card className="glass-card flex flex-col justify-between">
            ...
            <CardContent className="space-y-3 flex-1 flex flex-col justify-between">
              <Textarea
                ...
                placeholder={canEdit ? "Write onboarding notes, budget arrangements, key deliverables details..." : "No internal notes provided."}
        ...
        <ClientContactsTab
          ...
          onAddContact={async (contactData) => {
            ...
            await new Promise<void>((resolve) => {
              startTransition(async () => {
                const res = await createClientContact(client.id, contactData);
                if (res.ok) {
                  toast.success("Contact added successfully");
                  success = true;
                } else {
      ...
      <TabsContent value="documents" className="space-y-4">
        <ClientDocumentsTab
          ...
          canEdit={canEdit}
          isPending={isPending}
          onAddDoc={async (docData) => {
            let success = false;
            await new Promise<void>((resolve) => {
              startTransition(async () => {
                const res = await createClientDocument(client.id, docData);
                if (res.ok) {
                  toast.success("Document saved successfully");
                  success = true;
                } else {

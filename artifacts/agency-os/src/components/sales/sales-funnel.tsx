  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2 col-span-2">
          <Label>Lead Description / Research Notes</Label>
          <Textarea
            name="description"
            rows={6}
...
• Additional observations...`}
            className="text-xs resize-y w-full font-mono sm:font-sans whitespace-pre-wrap"
          />
        </div>
        <div className="space-y-2 col-span-2">
...
function LeadCard({ lead, canManage, onEdit, onDelete }: { lead: Lead; canManage: boolean; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="rounded-xl border bg-card p-3 text-sm shadow-sm hover:shadow-md transition-all group">
      ...
      {lead.description && (
        <p className="text-[11px] text-muted-foreground mt-2 line-clamp-3 whitespace-pre-wrap bg-muted/40 p-1.5 rounded border border-border/40 font-mono text-[10px]">
          {lead.description}
        </p>
      )}
      {lead.owner && (
